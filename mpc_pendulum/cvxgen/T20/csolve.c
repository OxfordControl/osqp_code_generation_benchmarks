/* Produced by CVXGEN, 2017-03-20 03:20:31 -0400.  */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "QN");
  if (xm == NULL) {
    printf("could not find params.QN.\n");
  } else {
    if (!((mxGetM(xm) == 4) && (mxGetN(xm) == 4))) {
      printf("QN must be size (4,4), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter QN must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter QN must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter QN must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.QN;
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 4) && (mxGetN(xm) == 1))) {
      printf("x_max must be size (4,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
